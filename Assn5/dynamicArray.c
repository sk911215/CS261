...
void addHeap(DynArr *heap, TYPE val, comparator  compare)
{
  /* FIXME: Write This */
    assert(heap != 0);
    addDynArr(heap, val);
    for (int i = sizeDynArr(heap)-1; i > 0; i = (i-1)/2)
        if (compare(getDynArr(heap, i), getDynArr(heap, (i-1)/2)) <= 0)
            swapDynArr(heap, i, (i-1)/2);
...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare)
{
  /* FIXME: Write this */
    ...
    if (rightChild < max) {
        ...
        if (compare(getDynArr(heap, pos), getDynArr(heap, smallestChild)) >= 0) {
            swapDynArr(heap, pos, smallestChild);
            _adjustHeap(heap, max, smallestChild, compare);
        }
    }
    else if(leftChild < max) {
        if (compare(getDynArr(heap, pos), getDynArr(heap, leftChild)) >= 0) {
