...
{
    /*FIXME: write this*/
    TaskP leftv = (TaskP)left;
    TaskP rightv = (TaskP)right;
    
    if (leftv->priority < rightv->priority)
        return -1;
    else if (leftv->priority > rightv->priority)
        return 1;
    else
        return 0;
}
...
{
    /*FIXME: Write this */
    TaskP v = (TaskP)malloc(sizeof(struct Task));
