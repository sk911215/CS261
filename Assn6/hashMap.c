...
void _setTableSize(struct hashMap * ht, int newTableSize)
{
    /*write this*/
    assert(ht != 0);
    hashLink **newtab = (hashLink**)malloc(sizeof(hashLink*) * newTableSize);
    assert(newtab != 0);
    int i;
    int idx;
    hashLink *cur;
    ...
    for(i=0; i < ht->tableSize; i++)
    {
        ...
        while(cur)
        {
            ...
            if(HASHING_FUNCTION == 1)
                idx = stringHash1(key) % newTableSize;
            else
                idx = stringHash2(key) % newTableSize;
            
            hashLink *newlnk = (hashLink *)malloc(sizeof(hashLink));
            assert(newlnk != 0);
            newlnk->key = cur->key;
            newlnk->value = cur->value;
            newlnk->next = newtab[idx];
            newtab[idx] = newlnk;
...
void insertMap (struct hashMap * ht, KeyType k, ValueType v)
{
    ...
    if(!containsKey ( ht, k))
    {
        char *key = (char*)k;
        if(HASHING_FUNCTION == 1)
            idx = stringHash1(key) % ht->tableSize;
        else
            idx = stringHash2(key) % ht->tableSize;
        
        hashLink *newlnk = (hashLink*)malloc(sizeof(hashLink));
        assert(newlnk != 0);
        newlnk->value = v;
        newlnk->key = k;
        newlnk->next = ht->table[idx];
        ht->table[idx] = newlnk;
        ht->count++;
    }
...
float tableLoad(struct hashMap *ht)
{
    /*write this*/
    float load;
    load = 1.0 * ht->count / ht->tableSize;
    return load;
}
